...
class MlPage(ctk.CTkFrame):
    def __init__(self, master):
        ...
        ctk.CTkLabel(left, text="测试集比例（决策树用，默认 0.2）", font=ctk.CTkFont(size=12)).grid(
    ...
    def _run(self):
        ...
        try:
            if op == OPS[0]:
                ...
                self._tree(self.data.get("1.0", "end"))
    ...
    def _tree(self, text):
        ...
        plot_tree(clf, filled=True, rounded=True, fontsize=9, feature_names=[f"特征{i+1}" for i in range(X.shape[1])], ax=self.ax)
        ...
        self._msg(f"测试集准确率 acc={acc:.1%}（{Xte.shape[0]} 个样本）")
        self._msg(f"特征重要性：{[f'{v:.3g}' for v in clf.feature_importances_]}")
        self._msg("提示：树的深度越大越容易过拟合，深度小更稳健。")